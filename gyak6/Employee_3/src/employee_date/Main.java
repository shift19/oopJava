...
public class Main {
	...
	public static void main(String[] args) {
		...
		// Setting static retiringAge variable
	...
	private static int[] readDate(Scanner sc) {
		...
		for (int i = 0; i < dArr.length; i++) {
			date[i] = Integer.parseInt(dArr[i]);
		}
		return date;
	}
	...
	private static void addEmployee(Scanner sc, Employee[] e, int i) {
		...
		if (c == 1) {
			...
			int[] birthday = readDate(sc);
			System.out.print("Enter salary: ");
			int salary = readInt(sc);

			e[i] = new Employee(name, birthday, salary);
		} else {
			System.out.print("Enter name: ");
			String name = readString(sc);
			System.out.print("Enter birthday: [YYYY,MM,DD] ");
			...
			e[i] = new Employee(name, birthday);
		}
	}
